...
const withDB = async (operations, res) => {
  ...
  try {
    ...
    const client = await MongoClient.connect(`mongodb+srv://$:${PASS}@cluster0.aqewv.mongodb.net/${DATA}?authSource=admin&replicaSet=atlas-qs1f2z-shard-0&w=majority&readPreference=primary&appname=MongoDB%20Compass&retryWrites=true&ssl=true`, { useNewUrlParser: true, useUnifiedTopology: true });
    const db = client.db(`shoestore`);   
    await operations(db);
    client.close();     
